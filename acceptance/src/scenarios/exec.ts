 ...
 * command exit (so callers can assert exit codes). Launch failures and configured timeouts reject;
 * timeout termination is delegated to child_process.execFile rather than an untracked Promise timer.
 */
export function execInSpawn(
  cfg: ExecConfig,
  identity: Identity,
  spawnId: string,
  cmd: string[],
  ...
  return new Promise((resolve, reject) => {
    const callback = (err: Error | null, stdout: string | Buffer, stderr: string | Buffer) => {
      if (err) {
        ...
        if (options.timeoutMs !== undefined && execError.killed) {
          reject(new Error(`execInSpawn: ${cfg.spawnctlBin} timed out after ${options.timeoutMs}ms`));
          return;
        }
    ...
    execFileCb(cfg.spawnctlBin, argv, {
      env: { ...process.env, XDG_CONFIG_HOME: cfg.configHome },
