...
var registry = map[string][]Runnable{
	...
	// stub is a deterministic, credential-free TEST FIXTURE binary (cmd/stubagent) used by the
	// multi-session e2e (sp-npxq.5). stub-acp serves canonical ACP (via acpmux) and echoes prompts
	// ("ECHO: <text>") with no model/sidecar — so the live e2e runs with no keys. NOT a real agent.
	"stub": {
