...
func TestNewManagerWithBackendInjectsBackendAndFloor(t *testing.T) {
	...
	if sp.SandboxID != "spz-sandbox" {
		t.Fatalf("SandboxID = %q, want spz-sandbox (injected backend not used)", sp.SandboxID)
	}
	if !fa.applied {
...
func TestNewManagerWithBackendAppliesDefaults(t *testing.T) {
	m := NewManagerWithBackend(fakeBackend(t), &fakeApplier{}, ManagerConfig{AgentImage: "a", SidecarImage: "s", DataRoot: t.TempDir()})
