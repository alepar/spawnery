...
// TestEndToEndHermes exercises the real Hermes agent (hermes-acp runnable) end to
// end through the spawnlet: an ACP round-trip (initialize -> session/new -> prompt)
// where the agent reads the secret out of the scratch mount and recites it, a
// multi-turn follow-up in the same session, and a suspend/resume-from-delta cycle
// proving the spawn relaunches from its captured rootfs delta and still answers.
//
...
// env is detected.
func TestEndToEndHermes(t *testing.T) {
	key := os.Getenv("OPENROUTER_API_KEY")
	if key == "" {
		t.Fatal("OPENROUTER_API_KEY is required for the Hermes e2e test")
	}

	rt, err := runtime.NewDocker()
	if err != nil {
		t.Fatalf("docker unavailable (need Docker for the Hermes e2e): %v", err)
	}
	...
	const (
		spawnID = "sp-hermes-e2e"
		model   = "openai/gpt-oss-120b:free"
		secret  = "QUOKKA-4417"
	)
	appPath := mustAbs(t, "../../examples/secret-app")

	// Create the Hermes spawn directly on the manager: the CreateSpawn RPC has no
	// runnable field and would default to the image's opencode runnable.
	if _, err := mgr.CreateWithSelection(ctx, spawnID, appPath, model, "", "", 0,
		spawnlet.AgentSelection{RunnableID: "hermes-acp"}); err != nil {
		t.Fatalf("create (hermes-acp): %v", err)
	}
	// Best-effort teardown of whatever pod is live at the end.
	defer func() {
	...
	func() {
		...
		// Phase 1: ACP round-trip — read the seeded secret via the file-read tool.
		...
		t.Logf("hermes turn1 reply: %q", t1.String())
		...
		t.Logf("hermes turn2 reply: %q", t2.String())
	...
	func() {
		...
		t.Logf("hermes post-resume reply: %q", t3.String())
