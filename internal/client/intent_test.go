...
// intent_test.go covers the AM1 client-side validation in pollAndSign [AM1], the
// provisionWithIntent NACK-retry orchestration [AC1], and BuildSessionOpenIntent.
...
func TestProvisionWithIntentHappyPath(t *testing.T) {
	...
	if err := provisionWithIntent(context.Background(), ic, "sp-happy", IntentParams{}, doRPC, nil); err != nil {
...
func TestProvisionWithIntentRetryableNACKRetries(t *testing.T) {
	...
	if err := provisionWithIntent(context.Background(), ic, "sp-retry", IntentParams{}, doRPC, warn); err != nil {
	...
	warnMu.Lock()
	gotWarnings := len(warnings)
	warnMu.Unlock()
	if gotWarnings == 0 {
		t.Fatal("expected the retryable-NACK warn callback to fire")
	}
}
...
func TestProvisionWithIntentNonRetryableNACKFails(t *testing.T) {
	...
	err := provisionWithIntent(context.Background(), ic, "sp-nonretry", IntentParams{}, doRPC, nil)
	if err == nil {
