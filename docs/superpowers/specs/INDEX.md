...
- [Spawn Fork (sp-li7h)](2026-06-14-spawn-fork-design.md) — clone a running spawn, both stay Active: `ForkSpawn` reuses the migration path but mints a new spawn id seeded into the fork's **own fully-isolated Kopia repo** (rehydrate + re-journal; full seed upload) — **v2 after roast BLOCK reverted the shared-repo "dedup-free seed", deferred to `sp-3y92`**; fork-point = live mount checkpoint + brief **agent-only** `docker pause` for the rootfs commit; `Forking` transient + recovery wiring for stranded-source safety; rootfs delta carries claude-code/codex conversation forward (`--continue`); cross-node via encrypted transfer set + fork-password delivery; opencode deferred (`sp-5h3.8`); status draft.
- [Spawn Fork — Adversarial Review (roast)](2026-06-14-spawn-fork-adversarial-review.md) — roast of the fork spec v1 → **BLOCK**: the shared-repo/shared-password lineage broke spawn isolation, the per-`(spawn,gen)` delete-fence, and Kopia's single-owner maintenance model (8 confirmed clusters; panel degraded by a monthly spend cap, 51/90 full panels). Drove the v2 revert to isolated per-fork repos + the split live/paused fork-point; shared-repo deferred to `sp-3y92`.
