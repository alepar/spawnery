...
- [User Secrets Store — Owner-Online Auto-Injection (sp-7h6.1)](2026-06-14-user-secrets-store-design.md) — durable CP-blind envelope store + `PutSecret`(CAS)/`GetSecret`/`ListSecrets` CRUD (unblocks the W3 sweep stub), typed entries (inference-key/generic-kv/generic github-token), owner-online-only delivery with the secrets-ready gate made explicit in the Create/Resume API, BYOK inference via the sidecar control endpoint, owner-declares-per-spawn least-privilege; **roast-revised** (decoupled from unbuilt sp-l5sx; in-flight replay guards scoped as sp-7h6.1.8; GitHub-App provisioning split to its own epic linked to sp-u53.1).
