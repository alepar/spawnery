...
- [Spawn Fork (sp-li7h)](2026-06-14-spawn-fork-design.md) — clone a running spawn, both stay Active: `ForkSpawn` reuses the migration path but mints a new spawn id + branches the journal as a **separate snapshot lineage inside a SHARED Kopia repo** (dedup-free seed; Kopia multi-writer confirmed) with lineage-refcounted teardown; brief `docker pause` checkpoint fork-point; rootfs delta carries claude-code/codex conversation forward (`--continue`); cross-node via owner-sealed delivery; opencode deferred (`sp-5h3.8`); status draft.
