...
- [GitHub Credentials + Storage Backend — Unified Design](2026-06-14-github-credentials-and-storage-unified-design.md) — unified `sp-v40s` + `sp-u53.1` contract: strict owner-online GitHub token custody (AS response-wrap handoff + memoryless refresh), CAS-versioned `github-token` tuple with clear metadata, structured fully-qualified repo bindings, targeted pre-pod `node-storage` unseal for GitHub mounts, REST+git backend mechanics, machine backstop refs under `refs/spawnery/backstop/<date>/<spawn>/<generation>/<branch>/<hash>`, owner-sealed-journal-aware suspend warnings, and user-directed generation-keyed recovery; status: draft/approved-for-roast; tags: github,secrets,storage.
