...
- [User Secrets Store — Owner-Online Auto-Injection (sp-7h6.1)](2026-06-14-user-secrets-store-design.md) — durable CP-blind envelope store + `PutSecret`/`GetSecret` CRUD (unblocks the W3 sweep stub), typed entries (inference-key/github-token/generic-kv), owner-online-only delivery + secrets-ready gate, GitHub OAuth capture+refresh ceremony, owner-selects-per-spawn least-privilege scoping; feeds the artifact substrate.
