...
- [GitHub Credentials + Storage Backend — Unified Design](2026-06-14-github-credentials-and-storage-unified-design.md) — unified `sp-v40s` + `sp-u53.1` contract, revised 2026-06-15 post-roast: owner-online-to-bootstrap + node+AS-to-sustain custody (new AS mint API keeps App `client_secret` AS-side, node-retained refresh, `repository_id`-scoped short-lived tokens), single GitHub App with Spawnery-enforced create policy (no GitHub permission profiles), no-push-rails (agent owns real pushes; Spawnery writes only journal-excluded backstop machine refs under `refs/spawnery/backstop/<date>/<spawn>/<generation>/<branch>/<hash>`), Kopia-journal-is-durability (backstop = cross-node/human recovery), defined fail-closed outcomes + backstop enumeration/recovery; gated on empirical spikes (repo-create feasibility, new-repo coverage, revocation, scaled-AS nonce); status: revised-post-roast; tags: github,secrets,storage.
